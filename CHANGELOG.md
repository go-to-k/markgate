...
- feat(hasher): add hash: diff so base-branch churn stops invalidating markers by @go-to-k in https://github.com/go-to-k/markgate/pull/69
- chore: gitignore the throwaway worktrees created for subagent runs by @go-to-k in https://github.com/go-to-k/markgate/pull/75
- fix(config): reject hash: diff on a deps-only gate instead of discarding it by @go-to-k in https://github.com/go-to-k/markgate/pull/72
- fix(hasher): refuse a dead include glob instead of digesting the empty set by @go-to-k in https://github.com/go-to-k/markgate/pull/73
